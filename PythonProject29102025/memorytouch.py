...
while True:
    ...
    if "what" in user_input.lower() and "my name" in user_input.lower():
        ...
        if messages:
            last_output = messages[-1]
            if hasattr(last_output, "content"):
                name_value = last_output.content
            else:
                name_value = str(last_output)
            print("Agent: You said your name is", name_value)
        else:
            print("Agent: I don't know your name yet.")
        continue

    # Handle Weather command
    if user_input.lower().startswith("weather") or "weather in" in user_input.lower():
        try:
            parts = user_input.split()
            # Try to extract city name after 'weather' or 'in'
            if "in" in parts:
                city_index = parts.index("in") + 1
                city = " ".join(parts[city_index:])
            else:
                city = " ".join(parts[1:])
            if not city:
            ...
            print("Agent: Couldn't fetch the weather right now.", e)
            continue
